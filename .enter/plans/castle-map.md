...
4. Clicking a marker opens an `InfoWindow` (Google's standard popup look) with name, address, rating, and a "View on Google Maps" link.
...
- Radius = `min(halfViewportDiagonal, 50000)` meters (Nearby Search (New) hard cap), `includedTypes: ['castle']`, `maxResultCount: 20`, field mask limited to `displayName, formattedAddress, location, googleMapsUri, iconMaskBaseUri, iconBackgroundColor, rating, userRatingCount` to minimize billable fields.
...
- `src/components/castle-map/CastlePopupContent.tsx` — small helper that builds the InfoWindow DOM content (name, address, rating, "View on Google Maps" link) — kept separate to avoid a monolithic map file.
...
- [ ] Clicking a marker opens a single shared `InfoWindow` anchored to that marker with name, formatted address, rating (if present), and a "View on Google Maps" link (`googleMapsUri`).
...
- [ ] Clicking the castle marker opens a popup with name/address/rating/link; clicking elsewhere or another marker closes/moves the popup correctly.
